...
class UnionPerformanceTests: XCTestCase {
	...
	override func setUp() {
		resultsTen = Set<Int>(firstTen).union(Set<Int>(secondTen)).sorted();
		resultsThirty = Set<Int>(firstThirty).union(Set<Int>(secondThirty)).sorted();
		resultsHundred = Set<Int>(firstRandomHundred).union(Set<Int>(secondRandomHundred)).sorted();
	}
	
	// MARK: SetArray Size 10
	func testUnion10SetArray() {
		let firstInitial = SetArray<Int>(self.firstTen);
		let secondInitial = SetArray<Int>(self.secondTen);
		var workingSet = SetArray<Int>();
		self.measure {
			for _ in 0..<10000 {
				workingSet = firstInitial.union(secondInitial);
			}
		}
